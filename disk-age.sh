...
for i in a b c d e; do	# Gotta make this smarter
	...
	echo "########################################################"
        ...
        smartctl -a $i | egrep 'rpm|Device Model|Model Family|SATA'
	MONTHS=$(smartctl -A $i | grep "Power_On_Hours" | awk '{print "Power On Years: " $10%8760/8760*12}')
	MONTHS=$(smartctl -A /dev/sd$i | grep "Power_On_Hours" | awk '{print "Power On Years: " $10%8760/8760*12}')
	YEARS=$(smartctl -A $i | grep "Power_On_Hours" | awk '{print "Power On Years: " $10/8760}')
	...
	smartctl -A $i | grep "Temp" | awk '{print "Temp : " $10 " C"}'
        lsblk | grep "sd$i"
        lsblk | grep "$i"
	} 2>&1 | tee -a $TMP
done
